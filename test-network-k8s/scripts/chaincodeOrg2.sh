...
function chaincode_command_group() {
  ...
  if [ "${COMMAND}" == "deploy" ]; then
    ...
    log "Installing chaincode for org2"
    install_chaincode $@
    ...
    log "Committing chaincode for org2"
    commit_chaincode $@
...
function launch_chaincode_service() {
  ...
  cat kube/${org}/${org}-cc-template.yaml \
    ...
    | exec kubectl -n $ORG2_NS apply -f -

  kubectl -n $ORG2_NS rollout status deploy/${org}${peer}-ccaas-${cc_name}
