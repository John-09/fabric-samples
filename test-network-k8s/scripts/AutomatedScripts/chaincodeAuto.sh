...
function chaincode_command_group() {
  ...
  if [ "${COMMAND}" == "deploy" ]; then
    ...
    log "Installing chaincode for ${ORG_NAME}"
    install_chaincode $@
    ...
    log "Committing chaincode for ${ORG_NAME}"
    commit_chaincode $@
...
# orgs=($(ls /workspaces/fabric-samples/test-network-k8s/build/enrollments))
# org_count=$((${#orgs[@]} - 1))
...
function query_chaincode_metadata() {
  ...
  log ''
  log '${ORG_NAME}-Peer2:'
  export_peer_context ${ORG_NAME} peer2
  peer chaincode query -n $cc_name -C $CHANNEL_NAME -c $args
}
...
function launch_chaincode_service() {
  ...
  cat ${NAMESPACE}/kube/${org}/${org}-cc-template.yaml \
    ...
    | exec kubectl -n $NAMESPACE apply -f -

  kubectl -n $NAMESPACE rollout status deploy/${org}${peer}-ccaas-${cc_name}
