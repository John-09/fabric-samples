...
function chaincode_command_group() {
  ...
  if [ "${COMMAND}" == "deploy" ]; then
    ...
    log "Installing chaincode for org3"
    install_chaincode $@
    ...
    log "Committing chaincode for org3"
    commit_chaincode $@
...
function query_chaincode_metadata() {
  ...
  log ''
  log 'Org3-Peer2:'
  export_peer_context org3 peer2
  peer chaincode query -n $cc_name -C $CHANNEL_NAME -c $args
}
...
function launch_chaincode_service() {
  ...
  cat kube/${org}/${org}-cc-template.yaml \
    ...
    | exec kubectl -n $ORG3_NS apply -f -

  kubectl -n $ORG3_NS rollout status deploy/${org}${peer}-ccaas-${cc_name}
